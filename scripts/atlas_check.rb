...
axes_registry = load_yaml(File.join(ATLAS, 'registry', 'axes.yml'))
families_registry = load_yaml(File.join(ATLAS, 'registry', 'edge_families.yml'))
...
def axis_snapshot(fm, axis_ids, axis_fields)
  axis_ids.to_h { |axis_id| [axis_id, fm[axis_fields.fetch(axis_id)]] }
end
...
definition_files = Dir[File.join(ATLAS, 'definitions', '*.md')]
...
expected_nodes.each do |id, expected|
  fm = definitions[id]
  if fm.nil?
    error("atlas/definitions: missing generated node #{id}")
    next
  end
...
definitions.each do |id, fm|
  ...
  error("#{rel(fm['_path'])}: definition id is not generated by #{rel(File.join(ATLAS, 'registry', 'axes.yml'))}") unless expected_nodes.key?(id)
end

implication_files = Dir[File.join(ATLAS, 'implications', '*.md')]
implications = {}
...
expected_nodes.keys.each do |source_id|
  expected_nodes.keys.each do |target_id|
    ...
    if edge.nil?
      error("atlas/implications: missing #{source_id} -> #{target_id}")
      next
    end
...
implications.each do |(source_id, target_id), edge|
  ...
  error("#{rel(edge['_path'])}: implication pair is not generated by #{rel(File.join(ATLAS, 'registry', 'axes.yml'))}")
end

witness_files = Dir[File.join(ATLAS, 'witnesses', '*.md')].to_h do |path|
  fm = frontmatter(path)
  [fm['id'], fm.merge('_path' => path)]
end
...
implications.each_value do |edge|
  Array(edge['witnesses']).each do |witness_id|
    ...
    if witness.nil?
      error("#{rel(edge['_path'])}: witness #{witness_id.inspect} has no atlas/witnesses note")
...
families_by_id.each_value do |family|
  ...
  path = File.join(ATLAS, 'arguments', "#{target}.md")
  if File.exist?(path)
    fm = frontmatter(path)
    error("#{rel(path)}: id should be #{target}") unless fm['id'] == target
  else
    error("#{rel(File.join(ATLAS, 'registry', 'edge_families.yml'))}: missing argument note #{target}")
  end
end
...
if @errors.empty?
  ...
  puts "definitions=#{expected_nodes.length} implications=#{implications.length}"
  puts "statuses=#{status_counts.sort.map { |key, value| "#{key}=#{value}" }.join(' ')}"
  puts "families=#{family_counts.sort.map { |key, value| "#{key}=#{value}" }.join(' ')}"
else
