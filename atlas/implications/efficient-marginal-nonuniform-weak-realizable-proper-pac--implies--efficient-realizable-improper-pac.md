---
type: implication
id: efficient-marginal-nonuniform-weak-realizable-proper-pac__implies__efficient-realizable-improper-pac
...
family: spike-tilt-uniformization-plus-boosting
axis_delta:
  resource: same
...
result_origin: new
---
# Efficient Marginal-Nonuniform Weak Realizable Proper PAC Learning $\Rightarrow$ Efficient Realizable Improper PAC Learning

## Verdict

`true`.

The marginal-nonuniform weak source first uniformizes to distribution-free weak realizable improper learning, and classical boosting gives the strong improper target.
...
**Goal.** Derive Efficient Realizable Improper PAC Learning from Efficient Marginal-Nonuniform Weak Realizable Proper PAC Learning.
...
The $m(s)$-sample transcript laws under $P$ and $Q$ have constant total variation distance, because a sample sees the spike with probability only $O(m(s)\varepsilon)$. But every low-edge output that is wrong at $x^\star$ has nonpositive edge under $Q$, contradicting the source guarantee for the marginal $Q$. Hence the same learner has a uniform inverse-polynomial weak edge $\Omega(1/m(s))$ under every marginal. Because the target is improper, the proper outputs supplied by the source are admissible weak hypotheses.
...
**Boosting.** The uniform weak learner has inverse-polynomial advantage and polynomial sample/time bounds independent of the marginal. Schapire's realizable weak-to-strong boosting theorem combines its outputs by an improper vote, giving the efficient strong realizable improper target.
