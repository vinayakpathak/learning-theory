---
type: implication
id: efficient-marginal-nonuniform-weak-realizable-improper-pac__implies__efficient-agnostic-proper-pac
...
# Efficient Marginal-Nonuniform Weak Realizable Improper PAC Learning $\Rightarrow$ Efficient Agnostic Proper PAC Learning

## Verdict

`false`, under the assumption $\mathrm{NP}\nsubseteq\mathrm{RP}$.

The witness satisfies the source because strong distribution-free agnostic improper learning implies weak realizable improper learning, and distribution-free bounds imply marginal-nonuniform bounds. The target fails for the witness because an efficient proper agnostic learner, restricted to realizable distributions where the optimum error is $0$, would be an efficient proper realizable learner. The same 3-SAT reduction rules that out.
