---
type: definition
id: efficient-agnostic-improper-pac
title: Efficient Agnostic Improper PAC Learning
...
characterization: "The sample-complexity analogue is characterized by finite VC dimension; no general representation-sensitive characterization of polynomial-time improper agnostic PAC learnability is recorded here."
...
# Efficient Agnostic Improper PAC Learning

## Definition

A learner receives i.i.d. examples from an arbitrary distribution $\mathcal D$ over $\mathcal X \times \{0,1\}$. With probability at least $1-\delta$, it must output a hypothesis $h$, possibly outside the benchmark class $\mathcal C$, such that
...
WHERE type = "implication" AND source = "efficient-agnostic-improper-pac"
SORT target ASC
```
...
FROM "atlas/implications"
WHERE type = "implication" AND target = "efficient-agnostic-improper-pac"
SORT source ASC
```
